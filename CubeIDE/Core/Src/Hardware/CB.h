...
#define SW_ON1_Port GPIOD
#define SW_ON2_Pin GPIO_Pin_1
#define SW_ON2_Port GPIOD
#define SW_ON3_Pin GPIO_Pin_2
#define SW_ON3_Port GPIOD
#define SW_ON4_Pin GPIO_Pin_3
#define SW_ON4_Port GPIOD
#define SW_ON5_Pin GPIO_Pin_4
#define SW_ON5_Port GPIOD
#define SW_ON6_Pin GPIO_Pin_5
#define SW_ON6_Port GPIOD
#define SW_ON7_Pin GPIO_Pin_6
#define SW_ON7_Port GPIOD
#define SW_ON8_Pin GPIO_Pin_7
#define SW_ON8_Port GPIOD


const uint8_t IndexToAddress[NUM_OF_CBS]        = {0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77};
const uint8_t IndexToCB[NUM_OF_CBS]             = {1   , 2   , 3   , 4   , 5   , 6   , 7   , 8   };        /* According to the active wires in each switch */
GPIO_TypeDef* const SwOnPort[NUM_OF_CBS]        = { SW_ON1_Port, SW_ON2_Port, SW_ON3_Port, SW_ON4_Port, SW_ON5_Port, SW_ON6_Port, SW_ON7_Port ,SW_ON8_Port};
const uint16_t SwOnPin[NUM_OF_CBS]              = { SW_ON1_Pin, SW_ON2_Pin, SW_ON3_Pin, SW_ON4_Pin, SW_ON5_Pin, SW_ON6_Pin, SW_ON7_Pin, SW_ON8_Pin };
#if (DASH_NUMBER == 100)
  const uint16_t output_max_overload_current[8]    = {(uint16_t)(25*16), 25*16, 25*16, (uint16_t)(15*16), 15*16, 15*16, 15*16, 15*16};
...
public:
  ...
  static uint8_t HandleGetStatusSWState (void);
  static uint8_t HandleGetStatusSWCmd   (void);
  static void    HandleGetStatusTripStat(uint8_t* trip_stat);
  static uint8_t HandleGetStatusBtlMode (void);
  static void    HandleGetStatusOutVolt (int16_t* out_volt); // ?
  static void    HandleGetStatusOutCurr (int16_t* out_volt);
...
private:
  ...
  static CB_Data CBs[NUM_OF_CBS];
  static Group_Data Groups[NUM_OF_GROUPS];
  static uint8_t CB_Groups_Int[NUM_OF_CBS];
