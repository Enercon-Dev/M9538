...
CB_Data CB_Int::CBs[NUM_OF_CBS]           = {0};
Group_Data CB_Int::Groups[NUM_OF_GROUPS]  = {0};
uint8_t CB_Int::CB_Groups_Int[NUM_OF_CBS] = {0,1,2,3,4,5,6,7};
Periodic_States_t CB_Int::periodic_state  = Periodic_States_t::Periodic_Begin;
uint8_t CB_Int::ConfigMask = 0;
uint8_t CB_Int::EnMask = 0;
...
ErrorStatus CB_Int::setGroup(uint8_t group_number)
{
  ...
  else
  {
    ...
    int i;
    for(i = 0; i < NUM_OF_CBS && CBs[i].GroupNum != group_number; ++i);
  ...
  Raw_Config_t rConf = {
                        CBs[CB_Index].Config.Over_Load_Limit,  
                        CBs[CB_Index].Config.Short_Circuit_Limit,
                        CBs[CB_Index].Thermal_Const,     
                        CBs[CB_Index].GroupNum
                        };
  return SystemManagement::getPersistentDataCenter().Set_CBSetting(CB_Index,&rConf);
}
...
void CB_Int::reorderGroups(uint8_t changed_index) {
  ...
  for(i = 0; i < NUM_OF_CBS && CB_Groups_Int[i] != changed_index; ++i);
  for(int k = i; k < NUM_OF_CBS; ++k) {
    CB_Groups_Int[k] = CB_Groups_Int[k + 1];
  }
  for(i = 0; i < (NUM_OF_CBS-1) && CBs[CB_Groups_Int[i]].GroupNum < group_num; ++i);
  for(int k = NUM_OF_CBS - 1; k > i; --k) {
    CB_Groups_Int[k] = CB_Groups_Int[k - 1];
  }
...
*/
uint8_t CB_Int::HandleGetStatusSWState()
{
  uint8_t state = 0;
  for (int i=0; i< NUM_OF_CBS; ++i) {
...
*/
uint8_t CB_Int::HandleGetStatusSWCmd()
{
  uint8_t cmd = 0;
  for (int i=0; i< NUM_OF_CBS; ++i) 
...
*/
uint8_t CB_Int::HandleGetStatusBtlMode()
{
...
ErrorStatus CB_Int::HandleSetSingleLimits(uint16_t ThrmConst, uint16_t Overload, uint16_t CurrLimit) { 
  ...
  raw_conf.Current_Limit   = CurrLimit;
  ...
  raw_conf.Thermal_Const   = ThrmConst;
...
void CB_Int::periodicCall(uint16_t InVoltage)
{
  ...
  case Periodic_States_t::Periodic_Status:
    ...
    if(GPIO_ReadOutputDataBit(SwOnPort[sw_index],SwOnPin[sw_index]) && Select(sw_index)->Full_Status_Read(&CB_Outputs) == SUCCESS) {
      CBs[sw_index].Meas_Current     = CB_Outputs.Current;
    ...
    else
    {
      ...
      sw_index = (sw_index + 1) % NUM_OF_CBS;
      if (sw_index == 0)
  ...
  case Periodic_States_t::Periodic_Config:
    if (IsBusy() == BusyStatus::SET)
      return;
    for (; sw_index<NUM_OF_CBS; sw_index++)
    {
