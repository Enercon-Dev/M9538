...
/* Function stores commLoss data in the eeprom */
ErrorStatus PersistentSystemData::Set_CommLossSetting(uint16_t Timeout, uint8_t Channels, uint8_t Mask)
{
  if (MemoryOk == ERROR)
    return ERROR;
  CommLoss_t comloss_set;
  comloss_set.ChannelMask       = Channels;
  comloss_set.ChannelsOperation = Mask;
  comloss_set.Timeout           = Timeout;
  return Memory.WriteToEEprom((&((SystemSettings_t *)SYSTEM_DATA_BASE_ADDRESS)->CommLoss), sizeof(CommLoss_t),&comloss_set);
}

/* Function reads commLoss data from eeprom and stores it in the given variables */
ErrorStatus PersistentSystemData::Get_CommLossSetting(uint16_t *Timeout, uint8_t *Channels, uint8_t *Mask)
{
  int rc = (int) SUCCESS;
  CommLoss_t comloss_get;
  if (MemoryOk == ERROR)
  {
    SystemSettings_t st;
    comloss_get = st.CommLoss;  
    rc &= ERROR;
  }
  else
    rc &= Memory.ReadFromEEprom((&((SystemSettings_t *)SYSTEM_DATA_BASE_ADDRESS)->CommLoss), sizeof(CommLoss_t),&comloss_get);
  *Timeout  = comloss_get.Timeout;
  *Channels = comloss_get.ChannelMask;
  *Mask     = comloss_get.ChannelsOperation;
  return (ErrorStatus)rc;
}
...
ErrorStatus PersistentSystemData::Set_PorSetting(const uint8_t *OrderSetting, const uint16_t *DelaySetting)
{
  if (MemoryOk == ERROR)
    return ERROR;
  int rc = (int) SUCCESS;
  Por_Order_t por_set[NUM_OF_CBS];
  for(uint8_t i = 0; i < NUM_OF_CBS; ++i)
  {
    por_set[i].Delay  = DelaySetting[i];
    por_set[i].Output = OrderSetting[i];
  }
  rc &= Memory.WriteToEEprom(((SystemSettings_t *)SYSTEM_DATA_BASE_ADDRESS)->Por_Order, sizeof(Por_Order_t) *NUM_OF_CBS ,&por_set);
  return (ErrorStatus) rc;
}
...
ErrorStatus PersistentSystemData::Set_DCISetting(const uint8_t *OrderSetting, const uint8_t *ActionSetting, const uint16_t *DelaySetting)
{
  if (MemoryOk == ERROR)
    return ERROR;
  int rc = (int) SUCCESS;
  DCI_Order_t dci_set[NUM_OF_CBS];
  for(uint8_t i = 0; i < NUM_OF_CBS; ++i)
  {
  ...
  rc &= Memory.WriteToEEprom(((SystemSettings_t *)SYSTEM_DATA_BASE_ADDRESS)->Dci_Order, sizeof(DCI_Order_t) *NUM_OF_CBS ,&dci_set);
  return (ErrorStatus) rc;
}
...
ErrorStatus PersistentSystemData::UpdateMemory(const SystemSettings_t* nSystemSettings)
{
  ...
  return Memory.WriteToEEprom(SYSTEM_DATA_BASE_ADDRESS, dataLen ,nSystemSettings);
}
...
{
  DataOut.writeShort(3+sizeof(SystemSettings_t)-20);
