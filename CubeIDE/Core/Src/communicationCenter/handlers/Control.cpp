...
void Controller::SendStatus(CONTROL_OPCODE  op, uint8_t broadcast){
  ...
  mBuffers.getOutBuffer().writeBlock(&Status_Msg , 62);
  ...
  case 1:
    if (length != 4)
    {
      ...
      return NACK;
       ...
       mBuffers.getOutBuffer().writeChar(0);
       mBuffers.getOutBuffer().writeChar(20);
...
{
  if (mBuffers.getInBuffer().readShort()!= (NUM_OF_CBS *3 +3)){
    return NACK;
  }
  uint8_t ChannelOrder[NUM_OF_CBS];
  uint16_t ChannelDelay[NUM_OF_CBS];
  for (int i=0; i< NUM_OF_CBS; i++)
...
{
  if (mBuffers.getInBuffer().readShort()!= (NUM_OF_CBS *4 +3)){
    return NACK;
  }
  uint8_t ChannelOrder[NUM_OF_CBS];
  uint8_t ChannelOperation[NUM_OF_CBS];
  uint16_t ChannelDelay[NUM_OF_CBS];
  for (int i=0; i< NUM_OF_CBS; i++)
    ChannelOrder[i] = mBuffers.getInBuffer().readChar();
  for (int i=0; i< NUM_OF_CBS; i++)
...
COMMAND_SUCCESS Controller::handleErase()
{
  ...
  if (strcmp(str, "ERASE EEPROM M9811-1") == 0){
    isReset = 1;
