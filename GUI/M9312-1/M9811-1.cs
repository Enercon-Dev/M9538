...
namespace M9811_1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            OUTPUT.OUTList.Find(x => x.OUT_ID == 1).OutLabel = Properties.Labels.Default.Label1;
            OUTPUT.OUTList.Find(x => x.OUT_ID == 2).OutLabel = Properties.Labels.Default.Label2;
            OUTPUT.OUTList.Find(x => x.OUT_ID == 3).OutLabel = Properties.Labels.Default.Label3;
            OUTPUT.OUTList.Find(x => x.OUT_ID == 4).OutLabel = Properties.Labels.Default.Label4;
            OUTPUT.OUTList.Find(x => x.OUT_ID == 5).OutLabel = Properties.Labels.Default.Label5;
            OUTPUT.OUTList.Find(x => x.OUT_ID == 6).OutLabel = Properties.Labels.Default.Label6;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private void ActiveFilter_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (e.PropertyName == "Status" || e.PropertyName == "")
            {
                foreach (OUTPUT sw in OUTPUT.OUTList)
                {
                    ...
                    sw.Voltage.Text = dev.Out_Volt[sw.OUT_ID - 1].ToString() + " V";
                    sw.Current.Text = dev.Out_Curr[sw.OUT_ID - 1].ToString() + " A";
                ...
                {
                    decimal current = 0;
                    gr.Outputs.ForEach(x => current += dev.Out_Curr[x.OUT_ID - 1]);
                    //TODO:ADD Group command status
                }
                lblInVolt.Text = dev.Input_Volt.ToString() + " V";
                lblInCurr.Text = dev.Input_Curr.ToString() +" A";
                lblTemp.Text = dev.Temp.ToString() + " °C";
            ...
            if (e.PropertyName == "Config")
            {
                ...
                iacSelectedGw.IPAddress = ActivePDU.DefaultGw;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            OutputsToProgram.Items.Clear();
            OutputsToProgram.Items.AddRange(OUTPUT.OUTList.Select(x => x.OUT_ID_Text).ToArray());
        ...
        private void menuItem3_Click(object sender, EventArgs e)
        {
            ...
            byte[] magic_check = new byte[13];
            fl.Seek(0x150, SeekOrigin.Begin);
            fl.Read(magic_check, 0, 13);
            if (string.Compare(System.Text.Encoding.Default.GetString(magic_check), "M9811-1 SW a") != 0)
            {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (!iacSelectedGw.AnyBlank)
                IpMode.DefaultGateWay = new IPAddress(iacSelectedGw.GetAddressBytes());
            else if (rbConfigStatic.Checked)
            {
                MessageBox.Show("Wrong IP Adress");
            }
            else
                IpMode.DefaultGateWay = new IPAddress(new byte[] { 0, 0, 0, 0 });
        ...
        {
            int[] channels = tbPorChans.Text.Split(',').Select(x => int.Parse(x)).ToArray();
            TimeSpan[] delays = tbPorDelay.Text.Split(',').Select(x => new TimeSpan(0,0,0,0,int.Parse(x))).ToArray();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                {
                    Erase.Confirm = "ERASE EEPROM M9811-1";
        ...
        private void dgvLimits_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                SetLim.Out(r.Channel, r.Overload, r.CurrentLimit, r.ThermalConst);
        ...
        private void btnDci_Click(object sender, EventArgs e)
        {
            try
            {
                int[] channels = tbDciChan.Text.Split(',').Select(x => int.Parse(x)).ToArray();
                TimeSpan[] delays = tbDciDelay.Text.Split(',').Select(x => new TimeSpan(0, 0, 0, 0, int.Parse(x))).ToArray();
                P_Enable[] operation = tbDciOper.Text.Split(',').Select(x => (P_Enable)(int.Parse(x))).ToArray();
                using (
        Set_Dci_Macro DCI = new Set_Dci_Macro()
        {
            Channel = channels,
            Delay = delays,
            State = operation
        ...
        private void btnGroupLimits_Click(object sender, EventArgs e)
        {
            using (Set_Limit SetLim = new Set_Limit())
            {
                SetLim.Group(GROUP.GROUPList.Find(x => x.GROUP_ID_Text.Equals(cbGroup.SelectedItem)).GROUP_ID, decimal.Parse(tbGroupOL.Text), decimal.Parse(tbGroupSC.Text), TimeSpan.FromMilliseconds( double.Parse(tbGroupTherm.Text)));
                SetLim.Send(ActivePDU);
            }
        }
        ...
        private void Btn_ChannelUpload_Click(object sender, EventArgs e)
        {
            Set_Debug msg = new Set_Debug();
            byte[] Index_arr = new byte[8];
            int programCount = 0;
            for(int i =0; i < 8; i++ )
            {
                bool isChecked = OutputsToProgram.GetItemChecked(i);
                if (isChecked)
                {
                    Index_arr[i] = (byte)i;
                    programCount++;
            ...
            if(programCount != 0)
            {
                ...
                {
                    //msg.CbToProgram = Index_arr[i];
                    //msg.Type = 2; //sending erase 0xdd
                    //msg.Send(ActivePDU);
                    //Thread.Sleep(2000);

                    //msg.Type = 3; // sending erase 0x7a
                    //msg.Send(ActivePDU);
                    //Thread.Sleep(10000);

                    //msg.Type = 4; // enable cb after erase
                    //msg.Send(ActivePDU);
                    //Thread.Sleep(1000);

                    //StreamReader f = new StreamReader(textBox3.Text);
