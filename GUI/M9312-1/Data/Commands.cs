...
namespace Data
{
    ...
    public class Message : IDisposable
    {
        ...
        protected readonly int NUM_OF_CB = 8;
    ...
    public class Status : Message
    {
        ...
        {
            get => Body.Skip(2).Take(NUM_OF_CB).Select(x =>
        ...
        public decimal[] Out_Volt
        {
            get
            {
                ...
                for (int i = 0; i < NUM_OF_CB; i++)
                    rc[i] = (decimal)(BitConverter.ToInt16(Body.Skip(12 + 2 * i).Take(2).Reverse().ToArray(), 0)/16.0);
                return rc;
            }
        }
        public decimal[] Out_Curr
        {
            get
            {
                decimal[] rc = new decimal[NUM_OF_CB];
                for (int i = 0; i < NUM_OF_CB; i++)
                    rc[i] = (decimal)(BitConverter.ToInt16(Body.Skip(28 + 2 * i).Take(2).Reverse().ToArray(), 0)/16.0);
                return rc;
            }
        }
        public int[] Channel_Group
        {
            get
            {
                int[] rc = new int[NUM_OF_CB];
                for (int i = 0; i < NUM_OF_CB; i++)
                    rc[i] = (int)(Body.Skip(44 + i).Take(1).First());
                return rc;
            }
        }
        public decimal Input_Volt
        {
            get => (decimal)(BitConverter.ToInt16(Body.Skip(52).Take(2).Reverse().ToArray(), 0) / 16.0);
        }
        public decimal Input_Curr
        {
            get => (decimal)(BitConverter.ToInt16(Body.Skip(54).Take(2).Reverse().ToArray(), 0) / 16.0);
        }
        public decimal Temp
        {
            get => (decimal)(BitConverter.ToInt16(Body.Skip(56).Take(2).Reverse().ToArray(), 0) / 16.0);

        }
        public Error_Stat_Item Error_Stat
        {
            get => Body.Skip(58).Take(1).Select(x =>
    ...
    public class Set_Group : Message
    {
        ...
        public int[] Channels
        {
            get
            {
                return Body.Skip(1).TakeWhile(x=> x!=0).Select(x => (int)x).ToArray();
            }
            set
            {
    ...
    public class Set_Por_Macro : Message
    {
        ...
        public TimeSpan[] Delay
        {
            set
            {
                ...
                for (int i = 0; i < value.Count() && i < 12; i++)
                {
    ...
    public class Set_Ip_Mode : Message
    {
        ...
        {
            set => SetToBody(value.GetAddressBytes(), 0, 9, 4);
        }
    }
    ...
    public class Config : Message
    {
        ...
        {
            if (Length != 132 && Length != 128) //TODO: update lenght
                throw new WrongParse("Wrong Length", this);
        ...
        public IPAddress DefGateWay
        {
            get => new IPAddress(Body.Skip(9).Take(4).ToArray());
        }

        public bool DynamicIP
        {
            get => Body.Skip(13).First() == 0 ? true : false;
        }
        ...
        public decimal[] OverloadSetting
        {
            get
            {
                ...
                for (int i = 0; i < NUM_OF_CB; i++)
                    val[i] = (decimal)(BitConverter.ToUInt16(Body.Skip(14 + i * 2).Take(2).Reverse().ToArray(), 0)/16.0);
                return val;
            }
        }
        ...
        public decimal[] CurrentLimitSetting
        {
            get
            {
                decimal[] val = new decimal[NUM_OF_CB];
                for (int i = 0; i < NUM_OF_CB; i++)
                    val[i] = (decimal)(BitConverter.ToUInt16(Body.Skip(30 + i * 2).Take(2).Reverse().ToArray(), 0) / 16.0);
                return val;
            }
        }
        ...
        public TimeSpan[] ThermalConstSetting
        {
            get
            {
                ...
                for (int i = 0; i < NUM_OF_CB; i++)
                    val[i] = TimeSpan.FromMilliseconds(BitConverter.ToUInt16(Body.Skip(46 + i * 2).Take(2).Reverse().ToArray(), 0) );
                return val;
            }
        }
        ...
        public TimeSpan[] PorChannelDelay
        {
            get
            {
                ...
                for (int i = 0; i < val.Length; i++)
                    val[i] = TimeSpan.FromMilliseconds(BitConverter.ToUInt16(Body.Skip(78 + 2*i).Take(2).Reverse().ToArray(), 0));
                return val;
            }
        }
        ...
        public P_Enable[] DciChannelAction
        {
            get
            {
                ...
                for (int i = 0; i < val.Length; i++)
                    val[i] = (P_Enable)Body.Skip(102 + i).First();
                return val;
            }
        }
        ...
        public TimeSpan[] DciChannelDelay
        {
            get
            {
                ...
                for (int i = 0; i < val.Length; i++)
                    val[i] = TimeSpan.FromMilliseconds(BitConverter.ToUInt16(Body.Skip(110 + 2 * i).Take(2).Reverse().ToArray(), 0));
                return val;
            }
        }
